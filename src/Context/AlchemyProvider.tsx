"use client";
import { Alchemy, AlchemySettings, BigNumber, Block, Network, TransactionReceipt } from "alchemy-sdk";
...
export const AlchemyProvider = ({ children }: PropsWithChildren) => {
  ...
  const findTransactionsReceipt= (receipts:TransactionReceipt[],transactionHash:string)=>{
    const transactionReceipt=receipts.find(tx=>tx.transactionHash===transactionHash);
    return transactionReceipt;
  }
  const getTransactionsFee = useCallback(async (blockHash:string):Promise<BigNumber>=>{
    let transactionsFee=BigNumber.from(0);
    const transactionsReceipts=await alchemy.core.getTransactionReceipts({blockHash});
    const BlockWithTransactions =await alchemy.core.getBlockWithTransactions(blockHash);
    if(!transactionsReceipts ||!BlockWithTransactions) return transactionsFee;
    ...
    for(const transaction of transactions ){
      ...
      const transactionReceipt= findTransactionsReceipt(receipts,transaction.hash);
